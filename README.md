...
구조분해할당 : 배열이나 객체의 속성을 분해해서 그 값을 변수에 담을 수 있게 해주는 것.
               비구조화 할당이라고도 한다.

만약 해당하는 값이 없으면 undefined가 들어간다.
ex) let [a, b, c] = [1 ,2]; => c에는 undefined가 들어감.

a, b, c에 기본값을 주면 undefined 대신 기본값이 들어간다.
ex) let [a=3, b=4, c=5] = [1 ,2]; =>  c = 5

객체 구조 분해 할당에서 let { name: 과일이름, brix: 당도 } = fruit; 이런 식으로 하면 
name이라는 키를 "과일이름"이라는 이름으로 사용할 수 있다. 
그래서 fruit.name의 값을 "과일이름"만으로 알 수 있다.

객체도 배열과 마찬가지로 해당하는 값이 없으면 undefined가 들어가며 기본값이 있다면 그 기본값이 들어간다.
...
![1](https://user-images.githubusercontent.com/105581499/194708200-bea4c8ef-b336-4c29-a105-abc288e02706.png)
